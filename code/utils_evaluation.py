...
def get_prediction(patches_pred_file: Path,
                   conf_thresholds: Dict[str, float]) -> str:
    """
    ...
    with patches_pred_file.open(mode="r") as patches_pred:
        ...
        for line in patches_pred_lines:
            ...
            if (line_class in classes) and (line_conf >
                                            conf_thresholds[line_class]):
    ...
    return f"{Path(patches_pred_file.name).with_suffix(f'.{config.args.image_ext}')}," \
           f"{max(class_to_percent.items(), key=operator.itemgetter(1))[0]}," \
           f"{','.join([f'{class_to_percent[_class]:.5f}' for _class in config.classes])}," \
           f"{','.join([f'{class_to_count[_class]:.5f}' for _class in config.classes])}"


def output_all_predictions(patches_pred_folder: Path, output_folder: Path,
                           conf_thresholds: Dict[str, float]) -> None:
    """
    ...
    with output_csv_path.open(mode="w") as writer:
        writer.write(
            f"img,predicted,"
            f"{','.join([f'percent_{_class}' for _class in config.classes])},"
            f"{','.join([f'count_{_class}' for _class in config.classes])}\n")

        csv_paths = get_csv_paths(folder=patches_pred_folder)
        for csv_path in csv_paths:
            writer.write(
                f"{get_prediction(patches_pred_file=csv_path, conf_thresholds=conf_thresholds)}\n"
            )


def grid_search(pred_folder: Path, inference_folder: Path) -> None:
    """
    ...
    """
    for threshold in config.threshold_search:
...
###########################################
def get_scores(gt_labels: Dict[str, str],
               prediction_labels: Dict[str, str]) -> Tuple[float, np.ndarray]:
    """
...
def find_best_acc_and_thresh(labels_csv: Path,
                             inference_folder: Path) -> Dict[str, float]:
    """
    ...
    for prediction_csv_path in prediction_csv_paths:
        ...
        avg_class_acc, conf_matrix = get_scores(
            gt_labels=gt_labels, prediction_labels=prediction_labels)
...
def print_final_test_results(labels_csv: Path, inference_folder: Path) -> None:
    """
...
def add_predictions_to_image(
        ...
        image: np.ndarray,
        prediction_to_color: Dict[str, np.ndarray]) -> np.ndarray:
    """
    ...
    for x, y in xy_to_pred_class.keys():
        ...
        start = round((0.9 * config.args.patch_size) / 2)
        end = round((1.1 * config.args.patch_size) / 2)
...
def visualize(wsi_folder: Path, preds_folder: Path, vis_folder: Path) -> None:
    """
    ...
    prediction_to_color = {
        config.classes[i]: color_to_np_color(color=config.colors[i])
        for i in range(config.num_classes)
    }
    # Go over all of the WSI.
    for whole_slide in whole_slides:
        # Read in the image.
        whole_slide_numpy = imread(uri=whole_slide)
